<template>
  <div class="container mx-auto mt-8 p-4 max-w-lg">
    ...
    <div v-if="comments.length === 0">Aucun commentaire pour l'instant.</div>
    <div v-for="comment in comments" :key="comment.id" class="bg-white rounded-lg shadow-md p-4 mb-4">
      ...
      <div v-if="comment.user.id === userId" class="mt-2">
        <button v-if="!editingComment || editingComment.id !== comment.id" @click="editComment(comment)" class="bg-yellow-500 text-white px-4 py-2 rounded">Modifier</button>
        <button v-else @click="saveComment(comment)" class="bg-green-500 text-white px-4 py-2 rounded">Enregistrer</button>
        <button @click="deleteComment(comment.id)" class="bg-red-500 text-white px-4 py-2 rounded ml-2">Supprimer</button>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      newComment: {
        content: '',
        userId: this.$auth.user ? this.$auth.user.id : null, // Assuming you have authentication set up
